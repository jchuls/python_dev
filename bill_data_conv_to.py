...
#---------------테이블 정의---------------
#영수증별 판매 내역
one_sales_table = "t_cjsmst"
#---------------테이블 정의---------------

succ_coll_mart = "select * from m_processing_log where mpl_msg = 'SUCC_LV2_DATA' and mpl_processing = '0' order by mpl_regDt ASC"
...
for mart_datainfo in coll_data:
    ...
    log_txts = "\n"+martidx+"작동시작 : " + str(now) + "\n"

    #월별 매출 데이터
    salse_Query = "SELECT moi_idx,DATE_FORMAT(sales_date,\"%Y-%m\") AS sales_date, SUM(tsale_amt) AS tsale_amt, SUM(dc_cpn_amt) as sale FROM " + one_sales_table + " where data_end = 0 and moi_idx = '"+martidx+"' GROUP BY moi_idx,DATE_FORMAT(sales_date,\"%Y-%m\")"
    ...
    for mdata in mart_info_data:
        ...
        if(data_space_result[0]['cnts'] == 0):
            data_space_insert = "insert into " + month_sales_table + " set mai_Company_num = '"+mdata['moi_idx']+"', mms_year = '"+dayexplode[0]+"', mms_"+dayexplode[1]+"m = '"+str(mdata['tsale_amt'])+"';"
        else:
            data_space_insert = "update " + month_sales_table + " set mms_"+dayexplode[1]+"m = '"+str(mdata['tsale_amt'])+"' where mai_Company_num = '"+mdata['moi_idx']+"' and mms_year = '"+dayexplode[0]+"';"
        dbfun.select(data_space_insert)

        #할인된 금액 데이터 
        data_space_check = "select count(*) as cnts from " + day_sales_table + " where mai_Company_num = '"+mdata['moi_idx']+"' and mms_year = '"+dayexplode[0]+"';"
        data_space_result = dbfun.select(data_space_check)
            
        if(data_space_result[0]['cnts'] == 0):
            data_space_insert = "insert into " + day_sales_table + " set mai_Company_num = '"+mdata['moi_idx']+"', mms_year = '"+dayexplode[0]+"', mms_"+dayexplode[1]+"m = '"+str(mdata['sale'])+"';"
        else:
            data_space_insert = "update " + day_sales_table + " set mms_"+dayexplode[1]+"m = '"+str(mdata['sale'])+"' where mai_Company_num = '"+mdata['moi_idx']+"' and mms_year = '"+dayexplode[0]+"';"
        dbfun.select(data_space_insert)
    ...
    #일자별 매출 데이터
    salse_day_Query = "SELECT moi_idx,DATE_FORMAT(sales_date,\"%Y-%m-%d\") AS sales_date, SUM(tsale_amt) AS tsale_amt FROM " + one_sales_table + " where data_end = 1 and moi_idx = '"+martidx+"' GROUP BY moi_idx,DATE_FORMAT(sales_date,\"%Y-%m-%d\")"
    ...
    for ddata in mart_info_day_data:
        ...
        if(data_space_result[0]['cnts'] == 0):
            data_space_insert = "insert into " + day_sales_table + " set mai_Company_num = '"+ddata['moi_idx']+"', mms_year = '"+dayexplode[0]+"', mss_month = '"+dayexplode[1]+"', mms_"+dayexplode[2]+"d = '"+str(ddata['tsale_amt'])+"';"
        else:
            data_space_insert = "update " + day_sales_table + " set mms_"+dayexplode[2]+"d = '"+str(ddata['tsale_amt'])+"' where mai_Company_num = '"+ddata['moi_idx']+"' and mms_year = '"+dayexplode[0]+"' and mss_month = '" + dayexplode[1] + "';"
        dbfun.select(data_space_insert)
    ...
    #사용할 일자별 변수 생성
    use_day_Query = "SELECT DATE_FORMAT(sales_date,\"%Y-%m-%d\") AS sales_date FROM " + one_sales_table + " where moi_idx='"+martidx+"' and data_end='2' GROUP BY DATE_FORMAT(sales_date,\"%Y-%m-%d\")"
    ...
    #일자별 객수 추출
    user_day_Query = "SELECT DATE_FORMAT(sales_date,\"%Y-%m-%d\") AS sales_date,moi_idx,COUNT(*) AS cnt FROM " + one_sales_table + " WHERE data_end = 2 and moi_idx = '"+martidx+"' GROUP BY moi_idx,concat(sales_date,pos_no)"
    ...
    salse_user_Query = "SELECT moi_idx,if(member_no = 0,0,1) as memno,DATE_FORMAT(sales_date,\"%Y-%m\") AS sales_date, SUM(tsale_amt) AS tsale_amt FROM " + one_sales_table + " where data_end = 3 and moi_idx = '"+martidx+"' GROUP BY moi_idx,if(member_no = 0,0,1),DATE_FORMAT(sales_date,\"%Y-%m\")"
    ...
    for userdata in mart_info_day_data:
        ...
        if(data_space_result[0]['cnts'] == 0):
            data_space_insert = "insert into " + mem_pay_table + " set mai_Company_num = '"+martidx+"', mms_year = '"+str(dayexplode[0])+"', mms_type = '"+str(userdata['memno'])+"', mms_"+str(dayexplode[1])+"m = '"+str(userdata['tsale_amt'])+"';"
        else:
            data_space_insert = "update " + mem_pay_table + " set mms_"+str(dayexplode[1])+"m = '"+str(userdata['tsale_amt'])+"' where mai_Company_num = '"+str(martidx)+"' and mms_year = '"+str(dayexplode[0])+"' and mms_type = '"+str(userdata['memno'])+"';"
    ...
    #1차가공 완료시 1과 함께 업데이트
    upprocessing = "update m_processing_log set mpl_processing = '1' where mpl_msg = 'SUCC_LV2_DATA' and mpl_idx = '"+str(mart_datainfo['mpl_idx'])+"'"
