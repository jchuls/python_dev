...
#시간기록
sys.stdout = open(r'C:\Users\youn\category\category_output_' + dt.last_mm + '.csv','w')
...
for mdata in mart_info_result:
    ...
    if not os.path.exists('Category_' + dt.last_mm + '.csv'):
        result_t.to_csv(r'C:\Users\youn\Category\Category_' + dt.last_mm + '.csv', index=False, mode='a', encoding='utf-8-sig', header=False)
    else:
        result_t.to_csv(r'C:\Users\youn\Category\Category_' + dt.last_mm + '.csv', index=False, mode='w', encoding='utf-8-sig', header=False)

csv_t = datetime.datetime.now()

#CSV Insert
fc = open(r'C:\Users\youn\Category\Category_' + dt.last_mm + '.csv',encoding='utf-8-sig')
...
for row in csvReader:
    ...
    sql = "insert into m_attmst(moi_idx, saup_code,attr1, attr2, attr3, attr_name, del_check, reg_date,ymd_chg) values ('" + str(
...
#작업확인 및 백업
sql_c = "SELECT COUNT(moi_idx) as cnt FROM m_attmst WHERE MID(ymd_chg, 1, 7) LIKE" '"%' + dt.last_m + '%"'
...
for row in open(r'C:\Users\youn\category\category_' + dt.last_mm + '.csv',encoding='utf-8-sig'):
 rowcount += 1

if result[0][0] == rowcount:
   shutil.move(r'C:\Users\youn\category\category_' + dt.last_mm + '.csv', r'C:\Users\youn\Backup\category')
...
#Log 기록
lg = open(r'C:\Users\youn\category\category_output_' + dt.last_mm + '.csv', encoding='cp949')
...
if result[0][0] == rowcount:
    sql_l = "insert into m_category_log(Log) values ('" + str(Log) + "');"
