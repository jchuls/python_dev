...
#시간기록
sys.stdout = open(r'C:\Users\youn\Point\Point_Output_' + dt.last_mm + '.txt','w')
...
for mdata in mart_info_result:
    ...
    #CSV 저장
    query = "SELECT B.no_saup, A.cd_byr,A.pnt_aprv,A.pnt_use,A.pnt_adj,A.knd_adj,A.ymd_reg,A.dt_sal,A.salpos,A.rcpno FROM mspnttrs A, msshpmst B WHERE MID(A.ymd_reg, 1, 7) LIKE" '"%' + dt.last_m + '%"'
    ...
    if not os.path.exists('Point_' + dt.last_mm + '.csv'):
        result_t.to_csv(r'C:\Users\youn\Point\Point_' + dt.last_mm + '.csv', index=False, mode='a', encoding='utf-8-sig', header=False)
    else:
        result_t.to_csv(r'C:\Users\youn\Point\Point_' + dt.last_mm + '.csv', index=False, mode='w', encoding='utf-8-sig', header=False)

csv_t = datetime.datetime.now()

#CSV Insert
fp = open(r'C:\Users\youn\Point\Point_' + dt.last_mm + '.csv',encoding='utf-8-sig')
...
for row in csvReader:
    ...
    sql = "insert into m_ptmst (moi_idx, saup_code,member_no,aprv_point,use_point,adj_point,adj_check,reg_date,sale_date,`sale_pos`,sale_pos_no) values ('" + str(
        moi_idx) + "','" + str(saup_code) + "','" + str(member_no) + "','" + str(aprv_point) + "','" + str(use_point) + "','" + str(
...
#작업확인 및 백업
sql_c = "SELECT COUNT(moi_idx) as cnt FROM m_ptmst;"
...
for row in open(r'C:\Users\youn\Point\Point_' + dt.last_mm + '.csv',encoding='utf-8-sig'):
 rowcount += 1

if result[0][0] == rowcount:
   shutil.move(r'C:\Users\youn\Point\Point_' + dt.last_mm + '.csv', r'C:\Users\youn\Backup\Point')
...
#Log 기록
lg = open(r'C:\Users\youn\Point\Point_output_' + dt.last_mm + '.csv', encoding='cp949')
...
if result[0][0] == rowcount:
    sql_l = "insert into m_point_log (Log) values ('" + str(Log) + "');"
